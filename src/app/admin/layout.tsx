...
export default function AdminLayout({children,}: Readonly<{ children: React.ReactNode; }>) {
    ...
    const {isLoaded, isSigned, isAdmin, user} = useUser()
    ...
    return (
        <div className={'relative _admin flex full'}>
            <div className={`opacity-0 transition-all ${isClosed ? 'min-w-0 w-0 max-w-0 ease-in-out duration-500' : 'ease-in min-w-32 max-w-64 duration-300'} h-full`}/>
            <div className={`fixed _menu transition-all duration-400 ${isClosed ? 'rounded-tr-2xl min-w-10 max-w-10 _menu-closed ease-in-out' : 'ease-in min-w-32 max-w-64 _menu-open px-4 rounded-tr-xl'} w-fit h-full flex flex-col border-r border-black/20 overflow-hidden`}>
                ...
                <div className={`relative _content transition-all flex flex-col items-center full gap-5 p-2 ${isClosed ? '-translate-x-[300%] ease-in-out duration-500' : 'translate-x-0 ease-in-out duration-300'}`}>
                    <div className={'flex-center flex-col rounded-2xl bg-white/10 overflow-hidden'}>
                        ...
                        <Link className={'w-full text-center py-1 border-t border-black/20 hover:border-black/0 hover:bg-white/5 transition-colors'} href={'/perfil'}>
                            Editar Perfil
                        </Link>
                    </div>
